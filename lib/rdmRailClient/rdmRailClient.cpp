...
void rdmRailClient::replaceWord(char* input, size_t inputSize, const char* target, const char* replacement) {
...
bool rdmRailClient::getCrsCode(const char *callingList, const char *crsList, const char *viaSearchLocation, char *crs) {
...
bool rdmRailClient::split_location(const char* src, const char* joiner, char* left, char* right) {
...
void rdmRailClient::value(const char *value) {
    if (fetchingDepartures) {
        if (strcmp(js->currentKey, "locationName")==0 && inCallingArray == 1) {
            ...
            return;
        } else if (strcmp(js->currentKey, "crs")==0 && inCallingArray == 1 && addedStopLocation) {
            ...
            return;
        } else if (strcmp(js->currentPath, "/operatorCode")==0) {
            strlcpy(xStation->service[id].opcoId,value,sizeof(xStation->service[0].opcoId));
            return;
        } else if (strcmp(js->currentPath, "/isCancelled")==0 && !inCallingArray) {
            ...
            return;
        } else if (strcmp(js->currentPath, "origin/crs")==0) {
            ...
            return;
        } else if (strcmp(js->currentPath, "destination/crs")==0) {
